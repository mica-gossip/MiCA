...
public class ClassUtils {
	...
	public static class RefCounter {
		...
		public boolean findReferenceCycles(Object obj) {
			...
			if (path == null)
				...
				path = Functional.prepend(
						path,
						new Reference(String.format("%s base object", obj
								.getClass().getCanonicalName()), obj));
				System.err.printf("Reference cycle found:\n");
				for (Reference r : path) {
					System.err.printf("  %s %s\n", r,
							(r.object == offendingObject ? "<-----" : ""));
				}
				return true;
			}
			...
			if (obj == null)
				...
				if(debug) {
					...
					if(obj instanceof Throwable) {
						System.err.println("      weirdness: cycle check object is a throwable; printing stack trace");
		...
		private List<Reference> getReferences(Object obj) {
			...
			for (Field field : k.getDeclaredFields()) {
				...
				try {
					temp.add(new Reference(String.format("%s.%s", k.getName(),
			...
			if (obj instanceof Iterable) {
				...
				for (Object element : (Iterable<Object>) obj) {
					temp.add(new Reference(String.format("%s.iterate[%d]",
							k.getName(), i++), element));
				}
			}
			...
			if (obj instanceof Object[]) {
				...
				for (Object element : (Object[]) obj) {
					temp.add(new Reference(String.format("%s.array[%d]",
							k.getName(), i++), element));
				}
			}

			if (obj instanceof Map) {
				temp.add(new Reference(
						String.format("%s.mapkeys", k.getName()), ((Map) obj)
								.keySet()));
				temp.add(new Reference(String.format("%s.mapvalues",
						k.getName()), ((Map) obj).values()));
			}
	...
	/**
	 * returns true if a reference cycle is found, and prints out a reference
	 * path on standard error
	 * 
	 * @param obj
	 * @return
	 */
	...
	public static byte[] getClassBytecode(final Class<?> klass)
			throws IOException {
	...
	/**
	 * Return the Exchange annotation proxy from a method, or null if none
	 * exists.
	 * 
	 * @param m
	 *            A method.
