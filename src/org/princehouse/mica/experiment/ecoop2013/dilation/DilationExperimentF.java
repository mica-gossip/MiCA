 ...
 * 
 */
public class DilationExperimentF extends TestHarness implements ProtocolInstanceFactory {

    public static void main(String[] args) {
        for (Protocol.Direction direction : new Protocol.Direction[] { Protocol.Direction.PUSHPULL,
                Protocol.Direction.PUSH, Protocol.Direction.PULL }) {

            for (int i = 0; i < 3; i += 2) {
                int dilation = i * i;

                DilationExperimentF test = new DilationExperimentF(dilation, direction);
    ...
    public Protocol createProtocolInstance(int nodeId, Address address, Overlay overlay) {
        Protocol p = new FindMinChatty(nodeId, overlay, direction, String.format("%s-dilation-%s", direction, dilation));
        if (dilation > 0) {
