...
public class Ecoop2013Independent extends TestHarness {
    ...
    private void megaDisruption() {
        ...
        Iterable<Address> addresses = Functional.map(runtimes, new F<MicaRuntime, Address>() {
            @Override
        ...
        for (MicaRuntime rt : runtimes) {
            ...
            MinAddressLeaderElection leader = (MinAddressLeaderElection) ((Merge) p.getP1()).getP1();
    ...
    private void beginTransientFaults() {
        ...
        for (int i = 0; i < tofail + 0; i++) {
            ...
            if (rt.getAddress().toString().equals("n0"))
                continue;
    ...
    public Protocol createProtocolInstance(int nodeId, Address address, Overlay overlay) {
        MinAddressLeaderElection leaderElection = new MinAddressLeaderElection(overlay);

        SpanningTreeOverlay tree = new SpanningTreeOverlay(leaderElection, overlay);

        TreeCountNodes counting = new TreeCountNodes(tree);
        TreeLabelNodes labeling = new TreeLabelNodes(counting);

        return merge.merge(merge.merge(leaderElection, labeling), merge.merge(tree, counting));
    }
