 ...
 *    int t = byzantine tolerance param.  number of nodes
 *            that must reply before advancing state = n - t
 *    
 * State at each node:
 *    sync - synchronizer state 
 *    queue - contains k replica stats
 *       r_i - replica state (specified separately)
 *       
 * 
 * 
 * @author lonnie
...
public class SynchronizerPipeline<S extends Synchronizer> extends BaseProtocol {
	...
	// length of ListMap is k+1, since the synchronizer's remote state is stored here too as the last column k
	...
	private void populateInitialReplicas() {
		// TODO need more thought about how this initialization is actually supposed to work
		for(int i = 0; i < k; i++) {
	...
	public void update(Protocol other) {
		...
		if(data.keySet().size() >= n - t) {
			// once we've collected data from enough neighbors, advance the synchronizer state and the pipeline
			...
			ListMapColumn<Address,Object> synchronizerData = dit.next(); // should be the last entry
