...
public class Pulse extends LogStructuredStateMachine {
			...
			.list(new LSSMTransitionRule[] {
					...
					new PulseTransitionRule(WAIT, COLLECT) {
						...
						public boolean readyPulse(Pulse node) {
							...
							return hasWaited(node, node.T3)
									&& node.census(WAIT, INDIRECT) >= node
											.getN() - node.getF();
						}
					...
					new PulseTransitionRule(READY, PROPOSE) {
						...
						public boolean readyPulse(Pulse node) {
							...
							return hasWaited(node, node.T4)
									|| node.census(PROPOSE, DIRECT) >= f + 1
									|| node.census(new PulseState[] { PROPOSE,
											RECOVER }, DIRECT) >= n - f;
						}
					},
					...
					new PulseTransitionRule(new PulseState[] { COLLECT, WAIT },
						...
						// within T3)
								...
								public boolean readyPulse(Pulse node) {
									int n = node.getN();
									int f = node.getF();
									return node.census(COLLECT, INDIRECT) >= n-f || 
											node.census(PULSE, DIRECT, node.T3) >= f+1;
								}
					},
					...
					new PulseTransitionRule(
							new PulseState[] { DOORWAY, ENTRY }, WAIT, PROPOSE) {
						...
						public boolean readyPulse(Pulse node) {
							int n = node.getN();
							int f = node.getF();
							return (hasWaited(node, node.T3) && node.census(new PulseState[]{ENTRY,DOORWAY}, INDIRECT) >= n-f) ||
	...
	public Pulse(Overlay overlay, int n, int f, int T4, int d,
			Object initialState) {
	...
	protected boolean Else() {
		// FIXME
		//    this should check that current transition rule is being tested AFTER all other transition rules from this source state that do not 
		//    invoke 'ELSE'.  Currently we rely on correct ordering of the transition rule definitions (rules are tested in the
	...
	/**
	 * Counts the number of nodes currently known to be in any of the given states, excluding the local node.
	 * Only messages with >= source attribution are considered
	 * @param states
	 * @param source
	 * @param within Within "within" milliseconds of current time. Value of zero means that within is ignored and entire known history is used
	 * @return
	 */
	...
	/**
	 * Return true if current node has been in any combination of sourceStates for the last t milliseconds 
	 * @param time
	 * @return
	 */
