 ...
 *   2. the addition of the method "protected boolean serializeField(Field field)", which can be used to selectively disable serialization of specific fields.
 */
...
/**
 * Serializes objects using direct field assignment. No header or schema data is
 * stored, only the data for each field. This reduces output size but means if
 * any field is added or removed, previously serialized bytes are invalidated.
 * If fields are public, bytecode generation will be used instead of reflection.
 * 
 * @see Serializer
 * @see Kryo#register(Class, Serializer)
 * @author Nathan Sweet <misc@n4te.com>
 */
public class ExtensibleFieldSerializer<T> extends Serializer<T> implements
        Comparator<ExtensibleFieldSerializer.CachedField> {
    final Kryo kryo;
    final Class type;
    ...
    public ExtensibleFieldSerializer(Kryo kryo, Class type) {
        this.kryo = kryo;
        this.type = type;
        rebuildCachedFields();
    }
    ...
    /**
     * Mechanism for subclasses of FieldSerializer to selectively serialize
     * fields. The specified field will not be serialized if this function
     * returns false.
     * 
     * @param field
     ...
     * called from the constructor for ExclusiveFieldSerializer, but not for
     * subclasses. Subclasses must call this from their constructor.
     */
    ...
    private CachedField newCachedField(Field field, int fieldIndex, int accessIndex) {
        ...
        cachedField.canBeNull = fieldsCanBeNull && !fieldClass.isPrimitive()
                && !field.isAnnotationPresent(NotNull.class);

        // Always use the same serializer for this field if the field's class is
        // final.
        if (kryo.isFinal(fieldClass) || fixedFieldTypes)
     ...
     * field's declared type. This allows ExclusiveFieldSerializer to be more
     * efficient, since it knows field values will not be a subclass of their
     * declared type. Default is false. Calling this method resets the
     * {@link #getFields() cached fields}.
     */
    ...
    // read and write are the relevant methods
    ...
    abstract class AsmCachedField extends CachedField {
        FieldAccess access = (FieldAccess) ExtensibleFieldSerializer.this.access;
    }
    ...
    class ObjectField extends CachedField {
        ...
        public void write(Output output, Object object) {
            try {
                ...
                if (valueClass == null) {
                    // The concrete type of the field is unknown, write the
                    // class first.
                    if (value == null) {
                    ...
                    // The concrete type of the field is known, always use the
                    // same serializer.
                    if (serializer == null)
                        this.serializer = serializer = kryo.getSerializer(valueClass);
                    if (generics != null)
                        serializer.setGenerics(kryo, generics);
                    if (canBeNull) {
        ...
        public void read(Input input, Object object) {
            try {
                if (TRACE)
                    trace("kryo", "Read field: " + this + " (" + type.getName() + ")");
                Object value;
