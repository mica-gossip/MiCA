...
public class ExtensibleFieldSerializer<T> extends Serializer<T> implements Comparator<ExtensibleFieldSerializer.CachedField> {
	final Kryo kryo;
	final Class type;
	...
	/**
	 * Mechanism for subclasses of FieldSerializer to selectively serialize fields.
	 * The specified field will not be serialized if this function returns false.
	 * @param field
	 ...
	 * will be used. It is called from the constructor for ExclusiveFieldSerializer, but not for subclasses. Subclasses must call this from
	 * their constructor. */
	protected void rebuildCachedFields () {
		...
		for (int i = 0, n = allFields.size(); i < n; i++) {
			...
			if(!serializeField(field)) continue;
	...
	/** Sets the default value for {@link CachedField#setClass(Class)} to the field's declared type. This allows ExclusiveFieldSerializer to
