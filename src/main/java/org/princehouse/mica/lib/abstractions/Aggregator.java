...
/**
 * Parametric aggregator class. This can be used to implement any protocol which follows the
 * pattern:
 * <p>
 * 1. Collect summaries from neighbors 2. Compute aggregate function of collected summaries
 * <p>
 * The Aggregator class caches the last known summary from each neighbor. This cache is updated with
 * each gossip exchange, and the aggregator recomputes the aggregate function.
 *
 * @param <AgClass>   Protocol class implementing the aggregator (i.e., the class you are writing)
 * @param <Summary>
 * @param <Aggregate>
 * @author lonnie
 */
public abstract class Aggregator<Summary, Aggregate> extends FailureDetector {
  ...
  /**
   * Aggregate constructor. The direction of aggregation is given by constants from
   * Protocol.Direction. PUSH : Push summary of initiating gossip node to recipient; compute new
   * remote aggregate PULL : Pull summary of recipient to initiator; compute new local aggregate
   * PUSHPULL: Both
   *
  ...
  /**
   * If summaryFilterKeep returns false, the specified summary will be purged from the summary
   * cache. Otherwise it is kept. Default behavior is to keep everything.
   *
   * @param addr
   * @param s
   * @return
