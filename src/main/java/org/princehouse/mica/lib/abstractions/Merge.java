 ...
 * <p>
 * Merge two protocols in such a way that the composite protocol gossips both subprotocols to the
 * same target peer as frequently as possible, while still respecting individual address
 * distributions.
 * <p>
...
public abstract class Merge extends BaseProtocol implements BinaryCompositeProtocol {
  ...
  public void unreachable(Address peer) {
    ...
    // to peer??
    if (getP1() != null) {
      getP1().unreachable(peer);
    }
    if (getP2() != null) {
      getP2().unreachable(peer);
    }
  }
  ...
  /**
   * Composite update function. Run both sub-updates if possible; otherwise run one or the other
   *
   * @param that
   */
  @GossipUpdate
  ...
  /**
   * Called to execute the update of MergeBase subclasses, including any merged subprotocols that
   * inherit MergeBase Includes a parameter for writing to the log. We only want the top composite
   * protocol to write to the log
   *
   * @param that
   * @param writeLog
   */
