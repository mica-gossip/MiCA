...
public abstract class NaiveBroadcast<Message extends Serializable> extends BaseProtocol implements
    Broadcast<Message> {
  ...
  public int ttl; // newly received messages will be broadcast this many times
  // before we stop sending them to neighbors
  ...
  // Seriously inefficient: Keep a set of all the message hashes we've ever
  // received, so we can compare them with incoming
  // messages
