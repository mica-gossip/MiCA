...
public class DemoTokenRingPipeline extends TestHarness implements ProtocolInstanceFactory {
  ...
  /**
   * Called by the test harness to generate a protocol instance (i.e., pipeline + leader election)
   * for each node.
   */
  @Override
   ...
   * @author lonnie
   */
  public static class TokenRingFactory extends Pipeline.ProtocolFactory<TokenRing> implements
      Serializable {
    ...
    public TokenRingFactory(SinglyLinkedRingOverlay ring, LeaderElection leader, int n,
        int nodeId) {
      this.ring = ring;
      this.n = n;
      this.leader = leader;
    ...
    public TokenRing createProtocol() {
      ...
      TokenRing tokenRing = new DemoTokenRing.ChattyTokenRing(ring, leader, networkSize,
          String.format(
