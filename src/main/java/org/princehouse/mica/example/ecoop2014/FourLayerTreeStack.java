...
/**
 * This is a merged four-protocol, self-stabilizing stack that sits on top of an arbitrary overlay,
 * "view" The main method demonstrates running an experiment with this protocol using the
 * TestHarness class to create many simulated nodes on a random graph
 * <p>
 ...
 * <p>
 * Layer 4: Node labeling. Gives nodes unique label in DFS traversal order of the tree. Uses the
 * subtree node count.
 * <p>
 * Protocol includes a main() method that can be used to run an experiment on the local machine. See
 * TestHarness.TestHarnessOptions for command line options.
 */
public class FourLayerTreeStack extends MergeCorrelated {
  ...
  /**
   * Initialize a demo protocol instance. Bootstrap parameters are a view and a unique node id used
   * for logging.
   *
   * @param view   An overlay instance. StaticOverlay can be used to run on a
   * @param nodeID fixed network graph. A unique id used for logging.
   */
