...
public class DemoTokenRing extends TestHarness implements ProtocolInstanceFactory {
    ...
    public MergeCorrelated createProtocolInstance(final int nodeId, Address address, Overlay overlay) {
        ...
        // A static ring overlay is provided by the test harness, but needs to
        // be cast
        ...
        /*
         * TokenRing needs an accurate notion of network size for correctness.
         * In this demo, network size is known ahead of time and is fixed, so we
         * use StaticNetworkSizeCounter to feed network size into the token
         * ring. In a more advance scenario, the NetworkSizeCounter could be a
         * separate protocol tasked with approximating network size.\
         * 
         * The ring overlay and network size counter do not need to be merged
         * because, being completely static, they are not protocols.
         */
        NetworkSizeCounter networkSize = new StaticNetworkSizeCounter(getOptions().n);

        // Create a ChattyTokenRing instance, feeding it ring / leader /
        // networkSize inputs, along with a triumphant
        // message to print when the token arrives.
        TokenRing tokenRing = new ChattyTokenRing(ring, leader, networkSize, String.format(
                "I have the token! (Node %s)", nodeId));
    ...
    /**
     * Run with TestHarness command line options. See TestHarness class or
     * details.
     * 
     * @param args
     */
    ...
    /**
     * Override the act() function of TokenRing to print a message to the
     * console when the token arrives.
     * 
     * @author lonnie
     * 
     */
    public static class ChattyTokenRing extends TokenRing {
        ...
        public ChattyTokenRing(SinglyLinkedRingOverlay overlay, LeaderElection leader, NetworkSizeCounter size,
                String msg) {
