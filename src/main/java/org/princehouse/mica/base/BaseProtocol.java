...
public abstract class BaseProtocol implements Protocol, Serializable {
  ...
  /**
   * RuntimeState stores information associated with a running protocol instance, such as the
   * current address and a random number generator seed.
   */
  @Override
  ...
  /**
   * Rate is a multiplier for how fast a node gossips. getRate() is called by the MiCA runtime to
   * determine how long to wait between gossips.
   */
  @Override
  ...
  /**
   * Get the current node's address. This is part of RuntimeState. This method is here for
   * convenience; it is the same as getRuntimeState().getAddress()
   *
  ...
  /**
   * Reserved for internal use. Write a message to the log. Log messages are comma-separated fields
   * of the format:
   * <p>
  ...
  /**
   * Convenience method for logging only an event type Flags is an enum from LogFlag enum.
   *
   * @param flags     An enum from LogFlag class; describes the category of the log message.
   * @param eventType An arbitrary string that will be written in the "event_type" attribute of the
   *                  logged JSON message.
   */
  @Override
  ...
  /**
   * @param flags    An enum from LogFlag class; describes the category of the log message.
   * @param evenType An arbitrary string that will be written in the "event_type" attribute of the
   *                 logged JSON message.
   * @param obj      An object to include with the log message. MiCA will attempt to serialize the
   *                 object into JSON, but this may fail for custom data types.
   */
  @Override
  ...
  /**
   * The default rate of all protocols is 1.0. Override this only if you specifically want to make
   * this protocol gossip at a non-uniform rate (i.e., merge operators do this)
   *
  ...
  /**
   * Returns an object that will be serialized when logging state update messages. By default, this
   * object is the protocol instance itself, but overriding this method can serialize an arbitrary
   * object instead.
   */
  @Override
  ...
  /**
   * Called after getView, prior to initiating gossip. preUpdate will be called even if the sampled
   * address is null.
   */
  @Override
  ...
  /**
   * Called on the node that initiates gossip after it has completed executing the gossip exchange.
   */
  @Override
  ...
  /**
   * Called when MiCA fails to reach a peer for gossip, either due to timeout or connection refused
   */
  @Override
