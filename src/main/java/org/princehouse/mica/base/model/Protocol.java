...
/**
 * Don't use this directly. Extend BaseProtocol to implement your own protocols.
 *
 * @author lonnie
 */
public interface Protocol {
  ...
  /**
   * RuntimeState is location-specific state that is independent of the protocol instance. This
   * includes the runtime clock and random number generator.
   *
  ...
  /**
   * Called by the runtime on the gossip initiator immediately after the update is executed This is
   * called even if the update method terminated with an exception or connection failure. I t can be
   * thought of as a "finally" block for update
   */
  ...
  /**
   * Returns an object that represents local node state. By default, the instance itself. This will
   * be JSON-serialized into the log. Circular references cause serialization to break; in that
   * case, this function will need to be overridden or the offending fields marked as transient.
   *
