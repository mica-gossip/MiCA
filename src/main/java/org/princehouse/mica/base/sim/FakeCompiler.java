...
public class FakeCompiler extends Compiler {
  ...
  /**
   * When running experiments with all nodes in one JVM, use a side channel to pass state between
   * sender and receiver. A null value is serialized.
   * <p>
   * note that f1, f2, and f3 for a gossip exchange must all be called in succession --- if any
   * other exchanges f1/f2/f3 are interleaved within the same thread, corruption will result.
   *
   * @author lonnie
   */
  public static class FakeCommunicationPatternAgent implements CommunicationPatternAgent {
    ...
    public Serializable f2(MicaRuntime receiverRuntime, Serializable m1)
      ...
      MiCA.getRuntimeInterface().getRuntimeContextManager()
          .setForeignRuntimeState(i.getProtocolInstance(), i.getRuntimeState());
      try {
        i.getProtocolInstance().update(receiverRuntime.getProtocolInstance());
      } catch (Throwable t) {
