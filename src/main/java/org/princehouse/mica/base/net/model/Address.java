...
/**
 * Interface representing the general concept of an Address. See TCPAddress for a TCP/IP specific
 * address.
 *
 * @author lonnie
 */
public interface Address extends Serializable, Comparable<Address> {
  ...
  /**
   * If address is local, open it for listening. Throw an AlreadyBoundException if address is
   * already bound
   * <p>
   * An address may only
   *
  ...
  /**
   * If bound, unbind (network may close the connection; unspecified) If unbound, throw
   * NotBoundException
   */
