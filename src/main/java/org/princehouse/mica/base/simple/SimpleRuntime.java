...
public class SimpleRuntime extends MicaRuntime implements AcceptConnectionHandler {
  ...
  /**
   * Entry point for SimpleRuntime. Starts a protocol in a new thread. (Calls through to launch(),
   * with the daemon flag true)
   *
   * @param pinstance  Local protocol instance
   ...
   * @return New Runtime instance
   */
  public static MicaRuntime launchDaemon(SimpleRuntime rt, final Protocol pinstance,
  ...
  public void acceptConnection(Address recipient, Connection connection)
      throws IOException, FatalErrorHalt,
      AbortRound {
  ...
  private <T extends Serializable> T receiveObject(CommunicationPatternAgent agent,
      Connection connection)
    ...
    try {
      ...
      if (offset != length) {
        ...
        throw new RuntimeException(
            String.format("EXPECTED %d bytes, read %d, ERROR\n", length, bytesRead));
      }
