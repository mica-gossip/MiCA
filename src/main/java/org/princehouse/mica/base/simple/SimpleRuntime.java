...
public class SimpleRuntime extends MicaRuntime implements AcceptConnectionHandler {
     ...
     */
    public static MicaRuntime launch(final MicaRuntime rt, final Protocol pinstance, final boolean daemon,
     ...
     * @return New Runtime instance
     */
    public static MicaRuntime launchDaemon(SimpleRuntime rt, final Protocol pinstance, final Address address,
    ...
    public void acceptConnection(Address recipient, Connection connection) throws IOException, FatalErrorHalt,
        ...
        CommunicationPatternAgent pattern = MiCA.getCompiler().compile(getProtocolInstance());
        ...
        try {
            if (lock.tryLock(getLockWaitTimeout(), TimeUnit.MILLISECONDS)) {
                ...
                try {
                    if (!running) {
                        logJson(LogFlag.error, "mica-error-internal", "acceptConnection called on a stopped runtime");
                        connection.close();
                        return;
                    }
                ...
                debug.printf("%s accept: failed to acquire lock (timeout)\n", this);
                connection.close();
            }
        } catch (InterruptedException e) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    handleError(RuntimeErrorCondition.CLOSE_CONNECTION_EXCEPTION, e);
                                }
                            }
    ...
    private <T extends Serializable> T receiveObject(CommunicationPatternAgent agent, Connection connection)
        ...
        try {
            ...
            int length = deserializeInteger(lengthBytes);
            ...
            if (offset != length) {
                ...
                throw new RuntimeException(String.format("EXPECTED %d bytes, read %d, ERROR\n", length, bytesRead));
            }
    ...
    @VisibleForTesting
    static byte[] serializeInteger(int i) {
