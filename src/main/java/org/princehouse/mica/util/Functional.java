...
public class Functional {
  ...
  /**
   * Turn an iterator into an iterable This can only be iterated over once; it iterator() is called
   * a second time, it throws an IteratorSpentException
   *
   * @param <T>
   * @param it
   * @return
  ...
  /**
   * Add everything from an iterable to a collection and then return the collection
   *
   * @param collection
   * @param iterable
   * @return
   */
  public static <C extends Collection<T>, T, S extends T> C extend(C collection,
      Iterable<S> iterable) {
    for (T q : iterable) {
      collection.add(q);
    }
  ...
  /**
   * Return the supplied iterable as a java.util.List. If it is already a list, return it instead of
   * creating a new one.
   */
   ...
   * @return
   */
  public static <S, T> Iterable<P2<S, T>> zip(final Iterable<S> s, final Iterable<T> t) {
  ...
  /**
   * Given an iterable [a,b,c,d] returns an iterable of pairs [ (a,b), (b,c), (c,d) ]
   *
   * @param <T>
   * @param it
   * @return
  ...
  public static <T> Iterable<P2<T, T>> segments(final Iterable<T> it) {
    return new Iterable<P2<T, T>>() {
      ...
      public Iterator<P2<T, T>> iterator() {
        ...
        return zip(iteratorAsIterable(i1), iteratorAsIterable(i2)).iterator();
      }
    };
  }
  ...
  /**
   * Map the segments of an iterable using a function (calls segments() to get the segments)
   *
   * @param <S>
   * @param <T>
   * @param s
   ...
   * @return
   */
  public static <S, T> Iterable<T> mapSegments(final Iterable<S> s, final F2<S, S, T> f) {
   ...
   * @return
   */
  public static <S, T> Iterable<T> mapParallel(Iterable<S> input, final F<S, T> f) {
    ...
    if (!MULTITHREADED) {
      ...
      int n = inputl.size();
      assert (n > 0);
      ExecutorService pool = Executors.newFixedThreadPool(Math.min(n, N_THREADS));
  ...
  /**
   * Return the first object from the list for which f evaluates to true Returns Option.none if
   * none
   *
   * @param l
   * @param f
  ...
  public static <A, B> HashMap<B, List<A>> groupBy(List<A> list, F<A, B> keyfunc) {
  ...
  public static <A> F<A, Boolean> or(final F<A, Boolean> a, final F<A, Boolean> b) {
  ...
  public static <A> F<A, Boolean> and(final F<A, Boolean> a, final F<A, Boolean> b) {
    return new F<A, Boolean>() {
      @Override
      public Boolean f(A arg) {
        return a.f(arg) && b.f(arg);
