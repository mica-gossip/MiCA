...
public class Randomness {
  ...
  public static int weightedChoice(Random r, double[] weights) {
    ...
    for (double w : weights) {
      if (w < -EPSILON) {
        throw new RuntimeException(String.format("subzero weight %f", w));
      }
      if (Double.isNaN(w)) {
    ...
    if (Math.abs(t - 1.0) >= EPSILON) {
      if (weights.length == 3) {
        System.out
            .printf("WEIGHTED CHOICE:   w = ( %f, %f, %f )\n", weights[0], weights[1], weights[2]);
      }
  ...
  /**
   * Copied verbose from stackoverflow thread http://stackoverflow.com/questions
   * /6001368/how-do-i-get-the-seed-from-a-random-in-java
   * <p>
  ...
  static public long getSeed(Random random) {
    ...
    return ((bar[i] & 255L) << 40 | (bar[i + 1] & 255L) << 32 | (bar[i + 2] & 255L) << 24
        | (bar[i + 3] & 255L) << 16 | (bar[i + 4] & 255L) << 8 | (bar[i + 5] & 255L))
        ^ 0x5DEECE66DL;
  }
